...
| 🌊 Qwen 流式调用 | ✅ 完成 | 替换 generate→stream，实时显示 AI 输出 | 当前未提交 |
...
| # | 功能 | 说明 | 预估 |
|---|------|------|------|
| 1 | **素材智能注入** | MaterialMatcher 4 步策略：类型映射→关键词→语义排序→人工确认 | 2-3 天 |
| 2 | **生成质量提升** | 叙述型章节用结构化 outline + 素材摘要增强 prompt | 1 天 |
| 3 | **端口统一** | 8000 端口释放后恢复，或配置为环境变量 | 0.5 小时 |

### P1 — 短期 (1-2 周)

| # | 功能 | 说明 | 预估 |
|---|------|------|------|
| 4 | **评分匹配优化** | 当前 evaluation 匹配依赖精确标题，改为语义相似度匹配 | 1 天 |
| 5 | **多招标文件格式** | 支持 .pdf（OCR）、.doc（转换） | 2 天 |
| 6 | **历史项目学习** | 从已完成的投标文件中学习公司写作风格 | 2-3 天 |
