...
{{
  "project_info": {{
    ...
    "lot_info": {{
      ...
      "lot_names": ["标段一"],
      "current_lot": "标段一",
      "description": "标段描述（如：标段一为境内法律服务，标段二为涉外法律服务）"
    }}
  }},
  ...
  "evaluation_criteria": [
    {{
      ...
      "material_evidence": "材料依据（评分表中该项要求提供的证明材料，原文摘录）",
...
6. evaluation_criteria 必须深度提取：
   ...
   - 如果评标办法以表格形式呈现，逐行提取
   - ⚠️ material_evidence（材料依据）必须从评分表中提取，原文摘录"材料依据"列内容
     例如："提供律所简介、基本情况与财务情况表、财务审计报告及相应证明材料"
7. 标段识别：如果招标文件涉及多个标段（标段一、标段二...），必须识别：
   - lot_info 中的标段数量和名称
   - 注意：不同标段的评分标准可能不同（如标段一"分所覆盖"=国内分所，标段二=境外办公室）
   - 只提取当前标段（文件标题中指明的标段）的评分标准
8. 所有字段尽量填写，实在找不到的写 null
