...
async def generate_full_document(task_id: str, req: FullBiddingRequest):
    ...
    async def event_generator():
        ...
        # Collect all sections from all volumes (LLM-generated)
        llm_sections = []
        for volume in requirements.get("volumes", []):
            for section in volume.get("sections", []):
                llm_sections.append(section)

        # ── Build section list from tender attachments ──
        format_spec = task.get("format_spec", {})
        if format_spec.get("has_format_chapter") and \
                format_spec.get("attachments"):
            from app.core.skills.builtin.section_planner import (
                build_sections_from_attachments,
            )
            scoring = requirements.get("evaluation_criteria", [])
            all_sections = build_sections_from_attachments(
                format_spec,
                llm_sections=llm_sections,
                scoring_criteria=scoring,
            )
            logger.info(
                f"[generate-full] SectionPlanner: {len(all_sections)} "
                f"sections from {len(format_spec['attachments'])} attachments "
                f"(LLM had {len(llm_sections)})"
            )
        else:
            # No format chapter — use LLM sections as-is
            all_sections = llm_sections
